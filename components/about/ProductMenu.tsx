...
export default function ProductMenu() {
  const t = useTranslations("ProductMenu");
  ...
  const productLinks: ProductLink[] = [
    ...
    },
    {
      ...
      subItems: [
        { name: t("stainlessFlexibleIntro"), href: "/products/stainless-steel-flexible-hose#intro" },
        { name: t("stainlessFlexibleConfig"), href: "/products/stainless-steel-flexible-hose#custom-options" },
      ],
    },
    {
      ...
      subItems: [
        { name: t("ptfeSmooth"), href: "/products/ptfe-teflon-hose#r14" },
        { name: t("ptfeConvoluted"), href: "/products/ptfe-teflon-hose#convoluted" },
      ],
    },
    { name: t("r7Hose"), href: "/products/r7-thermoplastic-hose" },
    { name: t("steamHose"), href: "/products/steam-hose" },
    {
      name: t("industrialHose"),
    ...
    { name: t("toyox"), href: "/products/toyox" },
    ...
    { name: t("pipeService"), href: "/products/industrial-pipe-service" },
  ];

  // กางเมนูอัตโนมัติหากหน้าปัจจุบันอยู่ในหมวดหมู่ย่อย
  useEffect(() => {
    const activeParent = productLinks.find(
      (link) =>
        pathname === link.href ||
    ...
    <aside className="sticky top-24 hidden h-fit w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl lg:block">
      ...
      <nav className="flex flex-col">
        {productLinks.map((item, index) => {
          ...
          const isParentActive = isExactActive || isChildActive;
          ...
          return (
            <div
              key={item.href}
              className={`group relative flex flex-col transition-colors duration-300 border-b border-zinc-50 last:border-0 ${
                isParentActive
                  ? "bg-[#af3030]/5"
                  : index % 2 === 0
                  ? "bg-[#fafafa]"
            ...
            >
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 bg-[#af3030] transition-transform duration-300 origin-top ${
                  isParentActive ? "scale-y-100" : "scale-y-0 group-hover:scale-y-100"
                }`}
              />
              ...
              <div className="flex items-stretch justify-between w-full">
                <Link
                  ...
                  className={`flex-1 px-6 py-4 transition-all duration-300 ${
                    isParentActive
                      ? "text-[#af3030] pl-8"
                      : "text-zinc-700 group-hover:pl-8 group-hover:text-[#af3030]"
                  }`}
                >
                ...
                {hasSubItems ? (
                  <button
                    ...
                    className={`px-5 flex items-center justify-center transition-colors duration-300 ${
                      isParentActive ? "text-[#af3030]" : "text-zinc-400 group-hover:text-[#af3030]"
                    }`}
                  >
                    <svg
                  ...
                  <div className="px-5 flex items-center justify-center">
                    <svg
                      className={`h-5 w-5 transition-all duration-300 ${
                        isExactActive
                          ? "translate-x-0 text-[#af3030] opacity-100"
                          : "-translate-x-2 text-zinc-300 opacity-0 group-hover:translate-x-0 group-hover:text-[#af3030] group-hover:opacity-100"
                      }`}
                      fill="none" viewBox="0 0 24 24" stroke="currentColor"
                    >
                      ...
                      {item.subItems?.map((sub) => {
                        ...
                        return (
                          <Link
                            key={sub.href}
                            href={sub.href}
                            className={`group/sub flex items-center gap-3 px-10 py-2.5 text-[14px] transition-all duration-300 ${
                              isSubActive
                          ...
                          >
                            <div
                              className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${
                                isSubActive ? "bg-[#af3030] scale-125" : "bg-zinc-300 group-hover/sub:bg-[#af3030]"
                              }`}
                            />
                            {sub.name}
                          </Link>
